...
from .timing import contract, digest, normalize, step, relative_strength, instant
    ...
    for code, panel in panels.items():
        ...
        for e in frozen:
            ...
            for rule in cfg['group_A']['exits']:
                ...
                out['trades'].append({**t, 'group':'A', 'arm':rule, 'code':code, 'episode_id':e['id'], 'entry_source_hash':e['source_hash'], 'market':panel['market'], 'industry':panel.get('industry','unknown'), 'volatility':'unknown', 'regime':'unknown'})
        ...
        for i in range(cfg['warmup']-1, len(rows)):
            ...
            if not any(f['code']==code and f['fold']==fold for f in out['folds']):
                out['folds'].append({'code':code,'fold':fold,'train_end_index':start-61,'test_start_index':start,'test_end_index':end-1,'purge':60,'refit':False})
            f = card['indicators']
