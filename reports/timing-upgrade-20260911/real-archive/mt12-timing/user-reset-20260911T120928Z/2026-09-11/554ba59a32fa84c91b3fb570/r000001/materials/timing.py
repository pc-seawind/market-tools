...
def step(rows, state, cfg, *, channel, observed_at):
    ...
    m['atr_stop'] = max(prior_stop, candidate) if prior_stop is not None else candidate
    failure = False
    ...
    if b['close'] < m['atr_stop']: risks.append('ATR_trailing_close_breach')
...
def evaluate(panel, *, asof, previous=None, channel='TREND', cfg=None):
    ...
    try:
        ...
        if state:
            ...
            if not indexes:
                return {**out, **state['last_card'], 'state':state, 'idempotent':True, 'status':'ok'}
        else:
        ...
        card.update(old_entry=legacy, old_exit={'status':'exit_review' if f['below_ma60_5sessions'] else 'monitor', 'rule':'5_completed_closes_below_each_MA60'},
                    ...
                    atr_stop=state['monitor']['atr_stop']/rows[-1]['factor'],
        ...
        out['gaps'] = [k+'_RS_unknown' for k,v in card['rs'].items() if v['status']=='unknown']
