...
def collect(scope_path, out):
    ...
    for code,item in {**scope['candidates'],**scope['recommendations'],**scope['holdings']}.items():
        ...
        p={'code':code,'name':item.get('name'),'market':market,'calendar_coverage_through':str(started.astimezone(ZoneInfo({'CN':'Asia/Shanghai','HK':'Asia/Hong_Kong','US':'America/New_York'}[market])).date()),'sessions':sessions,'calendar_verified':g['verified'],
...
def _observe(bundle_path, scope_path, root, previous=None):
    ...
    cfg=contract()
    code_hashes={n:file_hash(Path(__file__).with_name(n)) for n in ('timing.py','timing_cli.py','timing_experiment.py')}
    run_id=digest([raw.decode(),file_hash(scope_path),file_hash(previous) if previous else None,cfg,code_hashes])[:24]
...
def main():
    ...
    c=sub.add_parser('compare');c.add_argument('--bundle',required=True);c.add_argument('--out',required=True);c.add_argument('--observation')
    ...
    if a.cmd=='collect': r=collect(a.scope,a.out)
        ...
        source=None
        if a.observation:
            m=json.loads(Path(a.observation).read_text());source=Path(a.observation).parent/m['result']['path']
            if file_hash(source)!=m['result']['sha256']: raise ValueError('observation_hash_mismatch')
            observation=json.loads(source.read_text())
            if observation['scope_epoch']!=bundle['scope_epoch']: raise ValueError('observation_scope_mismatch')
            bundle['entry_episodes']=[{'id':'hypothetical-forward-'+c['code'],'code':c['code'],'signal_date':c['state']['monitor']['origin_date'],
                                      'source_hash':file_hash(source),'basis':'forward_monitor_hypothetical_NOT_user_entry_NOT_recommendation'} for c in observation['cards'] if c.get('state')]
        r=compare(bundle)
        r.update(input_bundle_sha256=file_hash(a.bundle),frozen_entries=bundle.get('entry_episodes',[]),observation_source=str(source) if source else None)
        write_new(a.out,r);r={'out':a.out,'sha256':file_hash(a.out),'summary':r['summary']}
    else:
