...
基本面维度 (ROE / 净利 YoY / 负债率) 来自 tushare `fina_indicator` API,
按 ts_code per-stock 查. 300 只选股候选 × 1 call/股 = 300 次调用耗配额. 解决:
```bash
$ python3 fina_sync.py                    # 默认 top 1000 按市值 (覆盖 95%)
$ python3 fina_sync.py --limit=5500       # 全市场
$ python3 fina_sync.py --codes=600519.SH,000858.SZ  # 指定股
```
Parquet 双写 via `cache_parquet`, 每次选股时 DuckDB 毫秒查. 推荐每月/每季
sync 一次 (财报发布后).
