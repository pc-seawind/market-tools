...
每个20/40/60交易日窗口到期时，将参考价格观察结果及目标日期、调整后价格、
原始bar与panel摘要、实际观测时间保存到state.horizon_observations，并随快照归档。
120根滑动输入不再重新定位已滑出的监控起点，也不覆盖已保存结果。旧state未含
新字段时仅在当前可核验目标bar仍存在时补录；已丢失目标数据保持blocked，不猜值。

报告逐窗口显示状态，mt12-monitor事件携带horizons进入weekly索引。
部分完成仍pending；全部参考价格窗口完成后事件标archived，表示该批价格观察
完成，不是策略validated或持仓退出，后续风险卡仍更新。当前输入阻断时保留历史
已完成窗口，未完成窗口标blocked。原历史归档不覆盖、不补写。

测试使用固定合成时钟，避免2026-09-11的固定测试日历在隔日全部被正确的生产
日历刷新门禁阻断。生产日历检查未放宽。新增146次日步进/120根滑窗、部分成熟与
阻断、归档到跨周索引端到端测试；这是机制验证，不能冒充真实持仓已满60交易日。
