...
遇到真实未终结信号，run 对对应隔离账本调用原 run_worker 的有界 once 模式采集和消费真实执行证据。非合法窗口、休市或证据不全保留回执，不伪造 fill；没有信号不空跑执行采集。未部署定时器，错过窗口不会补造历史报价。

真实首个完整原型轮位于 real/（代码 7bb7bee）；后续改进线性校验和后继代切换后，最终核验轮另存 real-final/，不改写原型证据。最终验收命令以交付记录中的 real-final 和 synthetic-complete 为准。
